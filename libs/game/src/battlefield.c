...
static void sunk_ship(BattleField* bf, Ship* ship) {
    ...
    for (TermSizeType cy = y0; cy < y1; ++cy) {
        for (TermSizeType cx = x0; cx < x1; ++cx) {
            ...
            int* existing = NULL;
            MapEC ec = map_find(bf->field, (Coord){.x=cx, .y=cy}, &existing);
            if (ec == MAP_ERR_Ok) {
...
GameEC bf_place_ship(BattleField* bf, TermSizeType x, TermSizeType y, bool is_horizontal, ShipType type) {
    ...
    Ship ship = {.id = bf->id_counter, .current_size=type+1, .max_size=type+1, .x=x,.y=y,.is_horizontal=is_horizontal};
    MapEC ec = MAP_ERR_Ok;
