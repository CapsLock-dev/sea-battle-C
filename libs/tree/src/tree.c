...
TreeEC tree_free(Tree* t) {
    ...
    if (t->head == NULL) {free(t); return TREE_EC_Ok;}
...
static TreeEC node_add_entry(Tree* t, Node* n, DataEnvelope* key, DataEnvelope* value) {
    if (n->curr_entry_count >= TREE_MAX_ENTRY_COUNT) return TREE_EC_EntryOverflow;
...
TreeEC tree_insert(Tree* t, DataEnvelope* key, DataEnvelope* value) {
    ...
    if (t->head == NULL) {
        ...
        if (node_add_entry(t, n, key, value) != TREE_EC_Ok) {node_free(n); return TREE_EC_AllocationError;}
        t->head = n;
        return TREE_EC_Ok;
    }
    Node* parent = NULL;
    ...
    if (curr->curr_entry_count == TREE_MAX_ENTRY_COUNT) {
        ...
        if (new_root == NULL) {node_free(right); return TREE_EC_AllocationError;}
    ...
    while (curr != NULL) {
        if (curr->curr_entry_count == TREE_MAX_ENTRY_COUNT) {
            ...
            if (ec != TREE_EC_Ok) {node_free(right); return ec;}
            size_t index = 0;
            for (size_t i=0; i<parent->curr_entry_count; ++i) {
                if (t->key_type->cmp(parent->entries[i].key, mid.key) == CMP_EQUAL) index = i;
            }
        ...
        for (; i<curr->curr_entry_count; ++i) {
            ...
            switch (cmp_res) {
                case CMP_EQUAL: return TREE_EC_KeyExists;
                case CMP_WRONG: return TREE_EC_CompareFail;
                case CMP_LESS: goto exit_loop;
                case CMP_MORE: break;
            }
        }
        exit_loop:
...
static void merge_children(Node* parent, size_t index) {
    ...
    parent->entries[parent->curr_entry_count - 1] = (Entry){.key = NULL, .value = NULL};
    ...
    if (target->children[0] != NULL) {
        for (size_t i=0; i<=target->curr_entry_count; ++i) {
            main->children[main->curr_entry_count+1+i] = target->children[i];
        }
    }
...
static void fix_weak(Node* parent, size_t index) {
    if (parent == NULL || parent->children[index] == NULL || parent->children[index]->curr_entry_count >= 2) return;
    ...
    if (index > 0 && parent->children[index-1]->curr_entry_count >= 2) {
        ...
        left->entries[left->curr_entry_count-1] = (Entry){.key=NULL, .value=NULL};
        ++child->curr_entry_count;
        --left->curr_entry_count;
    } else if (index < parent->curr_entry_count && parent->children[index+1]->curr_entry_count >= 2) { 
        ...
        right->entries[right->curr_entry_count-1] = (Entry){.key = NULL, .value = NULL};
        if (child->children[0] != NULL) {
