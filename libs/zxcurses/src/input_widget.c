...
TUIError input_widget_add_entry(InputWidget* widget, const char* label,
    ...
    InputEntry* temp = realloc(widget->entries,
                               sizeof(InputEntry) * (widget->curr_entries + 1));
...
void input_widget_render(InputWidget* widget) {
    ...
    for (size_t i = 0; i < widget->curr_entries; ++i) {
        if (widget->selected == i) {
            panel_draw_text(widget->panel, 1, (TermSizeType)i + 1, ">",
                            COLOR_DEFAULT, COLOR_DEFAULT);
            panel_draw_text(widget->panel,
                            2 + (TermSizeType)strlen(widget->entries[i].name),
                            (TermSizeType)i + 1, widget->entries[i].value, fg,
                            bg);
        } else {
            panel_draw_text(widget->panel, 1, (TermSizeType)i + 1, "|",
                            COLOR_DEFAULT, COLOR_DEFAULT);
            panel_draw_text(widget->panel,
                            2 + (TermSizeType)strlen(widget->entries[i].name),
                            (TermSizeType)i + 1, widget->entries[i].value,
                            COLOR_DEFAULT, COLOR_DEFAULT);
        }
        panel_draw_text(widget->panel, 2, (TermSizeType)i + 1,
                        widget->entries[i].name, COLOR_DEFAULT, COLOR_DEFAULT);
    }
